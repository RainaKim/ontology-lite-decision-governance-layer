...
class NovaReasoner:
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = _DEFAULT_MODEL):
        ...
        self.model = model
        logger.info(f"Initialized NovaReasoner with model: {model}")

    def reason_about_goal_alignment(self, decision_data: Dict, company_goals: List[Dict]) -> Dict:
        """
        Use Nova to reason about which strategic goals this decision aligns with.
...
Be rigorous. Only map goals with genuine alignment. Low scores for weak connections."""
        ...
        try:
            ...
            # Nova might include markdown code blocks, so extract JSON
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                logger.warning("Could not extract JSON from Nova response, returning raw text")
                return {"raw_reasoning": reasoning_text, "mapped_goals": []}

        except Exception as e:
            logger.error(f"Nova goal alignment reasoning failed: {e}")
            return {"error": str(e), "mapped_goals": []}

    def reason_about_ownership_validity(self, decision_data: Dict, personnel: List[Dict]) -> Dict:
        """
        Use Nova to validate if decision owners are appropriate given personnel hierarchy.
...
{{
  ...
  "reasoning_summary": "overall analysis"
        ...
        try:
            ...
            logger.error(f"Nova ownership validation failed: {e}")
    ...
    def reason_about_governance_conflicts(self, triggered_rules: List[Dict], decision_data: Dict,
        ...
        """
        Use Nova to resolve governance rule conflicts and optimize approval chain.
...
{{
  ...
  "reasoning_summary": "comprehensive analysis"
        ...
        try:
            ...
            logger.error(f"Nova governance reasoning failed: {e}")
    ...
    def reason_about_constraint_violations(self, decision_data: Dict, mapped_goals: List[Dict],
                                          company_data: Dict) -> Dict:
        """
        Use Nova to identify organizational constraint violations.
...
{{
  ...
  "reasoning_summary": "overall constraint analysis"
        ...
        try:
            ...
            logger.error(f"Nova constraint reasoning failed: {e}")
    ...
    def reason_about_graph_contradictions(
        ...
        """
        Extract relevant subgraph, then use Nova to find logical contradictions.
        ...
        try:
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                logger.warning("Could not extract JSON from Nova response")
                return {
            ...
            logger.error(f"Nova graph reasoning failed: {e}")
            return {
                "error": str(e),
                "contradictions": [],
                "recommendations": [],
    ...
    def _extract_mock_subgraph(
        ...
        TODO (Neo4j migration):
            ...
            create natural links between decisions:
                ...
                WHERE past.id <> $id
                RETURN past, r
            The Nova prompt already asks for historical analysis — it just
        ...
        if explicit_owners:
            ...
            # so Nova can reason about who should be accountable.
            for person in personnel:
                p_id = person.get("id")
    ...
    def _build_contradiction_prompt(self, decision_id: str, subgraph: Dict, lang: str = "ko") -> str:
        ...
        Organizes nodes by type so Nova can reason about structure clearly.
        """
